...
public:
    explicit PracticeTest1(int n)
        : BanditTest(std::make_shared<BanditPool>(std::initializer_list{
                std::shared_ptr<Bandit>(new Bandit(0.60, "Anna")),
                std::shared_ptr<Bandit>(new Bandit(0.25, "Boris")),
                std::shared_ptr<Bandit>(new Bandit(0.10, "Chris")),
            }
        ), n) {
    ...
    void LaunchTest() override {
        auto algorithms = std::vector<std::pair<std::shared_ptr<BaseBanditAlgorithm>, std::string>>{
            std::pair{std::make_shared<EpsilonGreedy>(this->pool_, 0.05), "e-greedy(0.05)"},
            std::pair{std::make_shared<Pursuit>(this->pool_, 0.05), "pursuit(0.05)"},
